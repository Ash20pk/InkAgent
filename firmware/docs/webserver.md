...
This guide explains how to use InkAgent's built-in web server for file
...
2. If you have saved Wi-Fi credentials, InkAgent first tries the last
...
- The mDNS fallback URL, usually `http://inkagent.local/`
...
3. Open the URL shown on the reader. `http://inkagent.local/` is preferred
...
2. Prefer `inkagent.local` when available, but keep the displayed IP address as a fallback.
