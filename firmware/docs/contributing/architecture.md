...
InkAgent is firmware for the Xteink X4 (unaffiliated with Xteink), built with PlatformIO targeting the ESP32-C3 microcontroller.
...
- `src/InkAgentSettings.h` (`SETTINGS`): user preferences and behavior flags
- `src/InkAgentState.h` (`APP_STATE`): runtime/session state such as current book and sleep context
...
Network file transfer is controlled by `src/activities/network/InkAgentWebServerActivity.h` and served by `src/network/InkAgentWebServer.h`.
