...
available while InkAgent is in File Transfer or Calibre Wireless mode.
...
Examples use `inkagent.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://inkagent.local/api/status
```
...
```bash
curl "http://inkagent.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://inkagent.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://inkagent.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://inkagent.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://inkagent.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://inkagent.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://inkagent.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://inkagent.local/delete
```
...
```bash
curl http://inkagent.local/api/settings
```
...
curl -X POST \
  ...
  http://inkagent.local/api/settings
```
...
```bash
curl http://inkagent.local/api/fonts
```
...
curl -X POST \
  ...
  http://inkagent.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://inkagent.local/api/fonts/delete
```
...
```bash
curl http://inkagent.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkagent.local/api/opds/delete
```
...
```bash
curl http://inkagent.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkagent.local/api/wifi/delete
```
...
- `inkagent.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `InkAgent-Reader`.
