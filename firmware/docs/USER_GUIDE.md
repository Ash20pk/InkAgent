...
Welcome to the **InkAgent** firmware. This guide outlines the hardware controls, navigation, and reading features of the device.

- [InkAgent User Guide](#inkagent-user-guide)
  ...
  - [3. Screens](#3-screens)
    ...
    - [3.5.1 Calibre Wireless Transfers](#351-calibre-wireless-transfers)
      ...
      - [Configuring the InkAgent Plugin in Calibre](#configuring-the-inkagent-plugin-in-calibre)
    ...
    - [3.6 Settings](#36-settings)
      ...
      - [3.6.7 KOReader Sync Quick Setup](#367-koreader-sync-quick-setup)
        - [Option A: InkAgent Sync Server (`sync.inkagentreader.com`, default)](#option-a-inkagent-sync-server-syncinkagentreadercom-default)
...
InkAgent supports sending books from Calibre using the InkAgent device plugin.

#### Installing the Plugin in Calibre

If you don't already have the plugin installed:

1. Head to https://github.com/inkagent-reader/calibre-plugins/releases to download the latest version of the inkagent_reader plugin.
2. Download the zip file.
3. Open Calibre → Preferences → Plugins → Load plugin from file → Select the zip file.
4. Restart Calibre.

#### Configuring the InkAgent Plugin in Calibre
1. In Calibre select Preferences.
2. In the Preferences dialog select Plugins.
3. In Plugins search for "inkagent".
...
To upload a book using the InkAgent plugin in Calibre:
...
The InkAgent plugin will connect to your device, create a folder for the book's author in the root folder (or the folder you configured for the plugin), then copy the book into that folder.
...
- **Sleep Screen**: Which sleep screen to display when the device sleeps:
  ...
  - "Dark" (default) - The default dark InkAgent logo sleep screen
...
- **Check for updates**: Check for InkAgent firmware updates over Wi-Fi. Firmware can also be updated without a USB connection by placing a `firmware.bin` file on the SD card.

- **Language**: Set the UI language. InkAgent supports 32 languages: English, Spanish, French, German, Czech, Brazilian Portuguese, European Portuguese, Russian, Swedish, Romanian, Catalan, Ukrainian, Belarusian, Italian, Polish, Finnish, Danish, Dutch, Turkish, Kazakh, Hungarian, Lithuanian, Slovenian, Valencian, Hebrew, Arabic, Slovak, Bosnian, Vietnamese, Norwegian Bokmål, Indonesian, and Orangutan.
...
InkAgent supports saving multiple OPDS servers and switching between them when browsing catalogs.
...
2. In a browser, open `http://<device-ip>/settings` or `http://inkagent.local`.
...
InkAgent can sync reading progress with KOReader-compatible sync servers.
It also interoperates with KOReader apps/devices when they use the same server and credentials.

##### Option A: InkAgent Sync Server (`sync.inkagentreader.com`, default)

When **Sync Server URL** is left empty, InkAgent uses the free InkAgent sync server at `https://sync.inkagentreader.com`. It speaks the standard KOReader sync protocol (so KOReader apps can use it too). InkAgent records page starts as chapter-content offsets and sends the corresponding standard KOReader XPath, so devices with different fonts or layouts can return to the same text.

1. On each InkAgent device:

   - Go to **Settings -> System -> KOReader Sync**.

   - Set **Username** and **Password** (enter the plain password; InkAgent computes MD5 internally, and use the same values on all devices).

   - Leave **Sync Server URL** empty (or set it to `https://sync.inkagentreader.com`).
...
Accounts are per server. Existing `sync.koreader.rocks` credentials do not exist on the InkAgent server; either sign up again with the same username/password or use Option B to keep using the legacy server.
...
1. On each InkAgent device:
   ...
   - Set **Sync Server URL** to `https://sync.koreader.rocks` (required; an empty URL now points at the InkAgent server instead).
...
3. Register a user once.
   InkAgent authenticates against KOReader Sync (`koreader/kosync`) using an MD5 key, so register using the MD5 of your password:
...
- With **Sync Behavior** set to **Smart sync**, InkAgent auto-resolves simple cases: upload when no remote progress exists, confirm and leave both unchanged when local and remote progress are already synchronized, upload when local progress is further ahead, or apply remote when remote progress is further ahead.
...
| Mode               | Behavior                                                                                                                     |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| **Dark** (default) | The InkAgent logo on a dark background.                                                                                    |
| **Light**          | The InkAgent logo on a white background.                                                                                   |
...
InkAgent supports loading additional fonts from the SD card, extending beyond the two built-in families (Noto Serif, Noto Sans). Custom fonts can include extended Unicode coverage, enabling CJK (Chinese, Japanese, Korean) and other scripts.
...
3. **Manual SD card copy:** Download font files from the [font pack repository](https://github.com/crosspoint-reader/crosspoint-fonts) and copy them to `/.fonts/` (preferred) or `/fonts/` on your SD card.
...
InkAgent renders text using the following Unicode character blocks, enabling support for a wide range of languages:
...
Bookmarks are stored in the `.inkagent/bookmarks` folder in the JSON format.
...
If an issue or crash is encountered while using InkAgent, feel free to raise an issue ticket and attach the logs.

**Crash reports on SD card:** After a crash, InkAgent automatically saves a crash report to the SD card (no USB connection needed). Check the root of the SD card for a crash log file and include it with any bug report.
...
There can be issues with broken cache or config. In this case, delete the `.inkagent` directory on your SD card (or consider deleting only `settings.json`, `state.json`, or `epub_*` cache directories in the `.inkagent/` folder).
