...
INKAGENT_VERSION for development environments.

Results in a version string like:  1.1.0-dev-feat-kosync-xpath-05c6cf8
Release environments are unaffected; they set INKAGENT_VERSION in the ini.
"""
...
def get_base_version(project_dir):
    ...
    if not config.has_option('inkagent', 'version'):
        warn('No [inkagent] version in platformio.ini; base version will be "0.0.0"')
        return '0.0.0'
    return config.get('inkagent', 'version')


def inject_version(env):
    # Only applies to development environments; release envs set the
    # version via build_flags in platformio.ini and are unaffected.
    if env['PIOENV'] not in ('default', 'sticky', 'screenshot'):
        return
    ...
    env.Append(CPPDEFINES=[('INKAGENT_VERSION', f'\\"{version_string}\\"')])
    print(f'InkAgent build version: {version_string}')
