...
OUTPUT_EPUB = ROOT / "InkAgent_User_Guide.epub"
...
def split_chapters(html: str):
    ...
    if not positions:
        return [('intro', 'InkAgent User Guide', html)]
...
def build_epub():
    ...
    book = epub.EpubBook()
    book.set_identifier('inkagent-user-guide-v1')
    book.set_title('InkAgent User Guide')
    book.set_language('en')
    book.add_author('InkAgent Project')
    ...
    # Full-size cover image (480×800) — used by InkAgent for home-screen thumbnail.
    ...
    # EPUB 2 cover declaration — InkAgent's Tier 1 OPF lookup
    book.add_metadata('OPF', 'meta', '', {'name': 'cover', 'content': 'cover-image'})

    # Cover page XHTML — first readable page in the spine
    cover_page = epub.EpubHtml(title='InkAgent User Guide', file_name='cover.xhtml', lang='en')
    cover_page.content = make_xhtml(
        'InkAgent User Guide',
        '<div class="cover-page">'
        '<img src="images/cover.png" alt="InkAgent User Guide cover"/>'
        '</div>',
    )
    ...
    # nav is excluded from the spine — InkAgent locates it via properties="nav" in
