...
namespace {
InkAgentPosition mappedPage(const int spine, const int page) {
  InkAgentPosition position{};
...
InkAgentPosition resolvedOffset(const int spine, const int page, const uint32_t offset) {
  InkAgentPosition position = mappedPage(spine, page);
...
TEST(ProgressComparison, PercentageFallbackRetainsTolerance) {
  const InkAgentPosition estimated{};
