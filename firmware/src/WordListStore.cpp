...
constexpr int32_t WordListStore::BOX_DAYS[5];
...
void WordListStore::add(const std::string& word, const std::string& book) {
  ...
  if (it != words.end()) {
    ...
    it->box = 0;
    it->due = today < 0 ? -1 : today + BOX_DAYS[0];
  ...
  w.added = today;
  w.due = today < 0 ? -1 : today + BOX_DAYS[0];
  w.box = 0;
  words.push_back(std::move(w));
  saveToFile();
}
...
std::vector<const WordEntry*> WordListStore::due(const int32_t today) const {
  ...
  for (const auto& w : words) {
    if (w.box >= kRetired) continue;
    // A word captured with no clock (due == -1) is always due: better to over-
    // offer a review than to lose the word because the RTC was flat.
    if (w.due < 0 || today < 0 || w.due <= today) out.push_back(&w);
  }
...
void WordListStore::grade(const std::string& word, const bool remembered, const int32_t today) {
  ...
  if (!remembered) {
    it->box = 0;
  } else if (it->box < kRetired) {
    it->box++;
  }
  if (today < 0) {
    it->due = -1;  // no clock: stays due, position in the box still advances
  } else if (it->box >= kRetired) {
    it->due = today + BOX_DAYS[BOX_COUNT - 1];
  } else {
    it->due = today + BOX_DAYS[it->box];
  }
  saveToFile();
}
