...
void pushRecentIndex(uint16_t* recentImages, uint8_t& recentPos, uint8_t& recentFill, uint16_t idx) {
  ...
  recentPos = (recentPos + 1) % InkAgentState::SLEEP_RECENT_COUNT;
  if (recentFill < InkAgentState::SLEEP_RECENT_COUNT) recentFill++;
}

}  // namespace

bool InkAgentState::isRecentSleep(uint16_t idx, uint8_t checkCount) const {
  return isRecentIndex(recentSleepImages, recentSleepPos, recentSleepFill, idx, checkCount);
}

bool InkAgentState::isRecentOverlaySleep(uint16_t idx, uint8_t checkCount) const {
  return isRecentIndex(recentOverlaySleepImages, recentOverlaySleepPos, recentOverlaySleepFill, idx, checkCount);
}

void InkAgentState::pushRecentSleep(uint16_t idx) {
  pushRecentIndex(recentSleepImages, recentSleepPos, recentSleepFill, idx);
}

void InkAgentState::pushRecentOverlaySleep(uint16_t idx) {
...
bool InkAgentState::fromJson(JsonVariantConst doc) {
  ...
  if (recentOverlaySleepPos >= SLEEP_RECENT_COUNT) {
    recentOverlaySleepPos = actualOverlayCount > 0 ? recentOverlaySleepPos % InkAgentState::SLEEP_RECENT_COUNT : 0;
  }
