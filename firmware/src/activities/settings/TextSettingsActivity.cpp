...
int findCurrentFontIndex(const SdCardFontRegistry* registry, const char* sdFontFamilyName, uint8_t fontFamily) {
  if (sdFontFamilyName[0] != '\0' && registry) {
    ...
    if (family != families.end()) {
      return InkAgentSettings::BUILTIN_FONT_COUNT + static_cast<int>(family - families.begin());
    }
  }

  return fontFamily < InkAgentSettings::BUILTIN_FONT_COUNT ? fontFamily : 0;
}
...
constexpr int MARGIN_MIN = InkAgentSettings::SCREEN_MARGIN_MIN;
constexpr int MARGIN_MAX = InkAgentSettings::SCREEN_MARGIN_MAX;
constexpr int MARGIN_STEP = InkAgentSettings::SCREEN_MARGIN_STEP;
}  // namespace
...
void TextSettingsActivity::onEnter() {
  ...
  fonts_.clear();
  fonts_.reserve(InkAgentSettings::BUILTIN_FONT_COUNT + (registry_ ? registry_->getFamilyCount() : 0));
  fonts_.push_back({I18N.get(StrId::STR_NOTO_SERIF), true, static_cast<uint8_t>(InkAgentSettings::NOTOSERIF)});
  fonts_.push_back({I18N.get(StrId::STR_NOTO_SANS), true, static_cast<uint8_t>(InkAgentSettings::NOTOSANS)});
  if (registry_) {
    const auto& families = registry_->getFamilies();
    for (int i = 0; i < static_cast<int>(families.size()); i++) {
      fonts_.push_back({families[i].name, false, static_cast<uint8_t>(InkAgentSettings::BUILTIN_FONT_COUNT + i)});
    }
  }
...
void TextSettingsActivity::rebuildSizeList() {
  ...
  for (const uint8_t pt : points) {
    ...
    // written the same way in every language InkAgent ships.
    char label[12];
...
void TextSettingsActivity::applyFamily(int listIndex) {
  ...
  if (font.isBuiltin) {
    ...
    const int sdIdx = font.settingIndex - InkAgentSettings::BUILTIN_FONT_COUNT;
    const auto& families = registry_->getFamilies();
