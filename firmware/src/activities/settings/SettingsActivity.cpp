...
void SettingsActivity::rebuildSettingsLists() {
  ...
  for (const auto& setting : getSettingsList(&sdFontSystem.registry(), &dictionaries)) {
    ...
    if (setting.category == StrId::STR_CAT_DISPLAY) {
      ...
      if (setting.valuePtr == &InkAgentSettings::pwrBtnFootnoteBack &&
          SETTINGS.shortPwrBtn != InkAgentSettings::SHORT_PWRBTN::FOOTNOTES) {
        continue;
      }
...
void SettingsActivity::toggleCurrentSetting() {
  ...
  const bool sleepScreenChanged = setting.valuePtr == &InkAgentSettings::sleepScreen;
  const bool quickResumeTimeoutChanged = setting.valuePtr == &InkAgentSettings::quickResumeSleepScreen;
  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::KOReaderSync:
        ...
        break;
      case SettingAction::AskBook:
        startActivityForResult(std::make_unique<InkAgentSettingsActivity>(renderer, mappedInput), resultHandler);
        break;
...
void SettingsActivity::syncQuickResumeTimeoutForSleepScreen(bool sleepScreenChanged, bool quickResumeTimeoutChanged) {
  ...
  if (SETTINGS.sleepScreen == InkAgentSettings::SLEEP_SCREEN_MODE::QUICK_RESUME) {
    if (SETTINGS.quickResumeSleepScreen != InkAgentSettings::QUICK_RESUME_SLEEP_SCREEN::QUICK_RESUME_AFTER_TIMEOUT) {
      SETTINGS.quickResumeSleepScreen = InkAgentSettings::QUICK_RESUME_SLEEP_SCREEN::QUICK_RESUME_AFTER_TIMEOUT;
  ...
  if (sleepScreenChanged && quickResumeTimeoutAutoEnabled && !preserveQuickResumeTimeoutOn) {
    SETTINGS.quickResumeSleepScreen = InkAgentSettings::QUICK_RESUME_SLEEP_SCREEN::QUICK_RESUME_NEVER;
    quickResumeTimeoutAutoEnabled = false;
  }
}
...
void SettingsActivity::openSleepTimeoutPicker() {
  startActivityForResult(
      std::make_unique<IntervalSelectionActivity>(
          ...
          InkAgentSettings::MIN_SLEEP_TIMEOUT_MINUTES, InkAgentSettings::MAX_SLEEP_TIMEOUT_MINUTES, 1, 5,
...
std::string SettingsActivity::settingValueText(const SettingInfo& setting) {
  ...
  if (setting.type == SettingType::VALUE && setting.valuePtr != nullptr) {
    if (setting.nameId == StrId::STR_TIME_TO_SLEEP) {
      if (SETTINGS.sleepTimeoutMinutes >= InkAgentSettings::SLEEP_TIMEOUT_NEVER_MINUTES) {
