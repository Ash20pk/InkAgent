...
#include "network/InkAgentWebServerActivity.h"
...
void ActivityManager::goToFileTransfer() {
  replaceActivity(std::make_unique<InkAgentWebServerActivity>(renderer, mappedInput));
}
...
void ActivityManager::goToReader(std::string path, const bool allowFastInitialRefresh) {
  ...
  if (FsHelpers::hasBmpExtension(path) || FsHelpers::hasPngExtension(path) || FsHelpers::hasJpgExtension(path) ||
      FsHelpers::hasXthExtension(path)) {
...
void ActivityManager::goHome(HomeMenuItem initialMenuItem, bool cleanInitialRefresh) {
  if (initialMenuItem == HomeMenuItem::NONE && currentActivity) {
    ...
    } else if (activityName == "InkAgentWebServer") {
...
bool ActivityManager::isHomeActivity() const {
  return std::any_of(stackActivities.begin(), stackActivities.end(),
                     [](const auto& activity) { return activity->isHomeActivity(); }) ||
