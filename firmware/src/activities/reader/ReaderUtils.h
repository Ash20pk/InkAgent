...
inline void applyOrientation(GfxRenderer& renderer, const uint8_t orientation) {
  switch (orientation) {
    case InkAgentSettings::ORIENTATION::PORTRAIT:
      renderer.setOrientation(GfxRenderer::Orientation::Portrait);
      break;
    case InkAgentSettings::ORIENTATION::LANDSCAPE_CW:
      renderer.setOrientation(GfxRenderer::Orientation::LandscapeClockwise);
      break;
    case InkAgentSettings::ORIENTATION::INVERTED:
      renderer.setOrientation(GfxRenderer::Orientation::PortraitInverted);
      break;
    case InkAgentSettings::ORIENTATION::LANDSCAPE_CCW:
...
inline PageTurnResult detectPageTurn(const MappedInputManager& input) {
  ...
  const bool powerTurn = SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::PAGE_TURN &&
...
inline TouchPageTurn detectTouchPageTurn(const GfxRenderer& renderer, const MappedInputManager& input) {
  ...
  if (SETTINGS.touchReaderControls == InkAgentSettings::TOUCH_READER_SWIPE) {
  ...
  const bool inverted = SETTINGS.touchReaderControls == InkAgentSettings::TOUCH_READER_INVERTED_TAP;
...
inline bool isTouchMenuTap(const GfxRenderer& renderer, const MappedInputManager& input) {
  ...
  if (SETTINGS.showReaderMenu != InkAgentSettings::READER_MENU_TAP) return false;
  int x = 0;
  int y = 0;
...
inline bool isTouchMenuGesture(const GfxRenderer& renderer, const MappedInputManager& input) {
  ...
  if (SETTINGS.showReaderMenu == InkAgentSettings::READER_MENU_SWIPE_UP && input.wasReaderMenuSwipeUp()) {
    return true;
  }
