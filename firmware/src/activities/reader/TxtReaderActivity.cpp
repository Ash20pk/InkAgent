...
bool TxtReaderActivity::loadBook() {
  txt = makeUniqueNoThrow<Txt>(bookPath, "/.inkagent");
  if (!txt) {
    ...
    for (const auto& line : currentPageLines) {
      if (!line.empty()) {
        ...
        switch (effectiveAlignment) {
          case InkAgentSettings::LEFT_ALIGN:
          default:
            break;
          case InkAgentSettings::CENTER_ALIGN: {
            x = cachedOrientedMarginLeft + (contentWidth - textWidth) / 2;
            break;
          }
          case InkAgentSettings::RIGHT_ALIGN: {
            x = cachedOrientedMarginLeft + contentWidth - textWidth;
            break;
          }
          case InkAgentSettings::JUSTIFIED:
            break;
        }
