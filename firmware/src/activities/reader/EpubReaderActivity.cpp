...
void moveFinishedBookToReadFolder(const std::string& srcPath, const std::string& dstPath,
  ...
  const std::string newCachePath = "/.inkagent/epub_" + std::to_string(std::hash<std::string>{}(dstPath));
...
bool EpubReaderActivity::loadBook() {
  auto loadedEpub = makeUniqueNoThrow<Epub>(bookPath, "/.inkagent");
  if (!loadedEpub) {
...
void EpubReaderActivity::loop() {
  ...
  if (confirmLongPressed) {
    switch (SETTINGS.longPressMenuFunction) {
      case InkAgentSettings::LP_MENU_BOOKMARK:
        addBookmark();
        showBookmarkMessage = true;
        bookmarkMessageTime = millis();
        requestUpdate();
        break;
      case InkAgentSettings::LP_MENU_KOSYNC:
        if (launchKOReaderSync()) {
          return;
        }
        break;
      case InkAgentSettings::LP_MENU_DICTIONARY:
        openDictionaryWordSelect();
        return;
      case InkAgentSettings::LP_MENU_READER_MENU:
      case InkAgentSettings::LP_MENU_DISABLED:
      default:
        break;
    }
  }
  ...
  if (SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::FOOTNOTES &&
  ...
  auto progressChangeResultHandler = [this](const ActivityResult& result) {
    ...
    if (result.isCancelled) {
      ...
      if (!cachedPageMatchesActiveSection && sync.hasSavedProgress) {
        ...
        InkAgentPosition fallback =
            ProgressMapper::toInkAgent(epub, {sync.xpath, sync.percentage}, renderer, currentSpineIndex, totalPages);
...
bool EpubReaderActivity::launchKOReaderSync() {
  ...
  InkAgentPosition localPos = getCurrentPosition();
...
bool EpubReaderActivity::launchAskBook() {
  if (!epub || !section || section->currentPage < 0 || section->currentPage >= section->pageCount) {
    LOG_ERR("ASKB", "No page to ask about (section=%d page=%d count=%d)", section ? 1 : 0,
            section ? section->currentPage : -1, section ? section->pageCount : -1);
    return false;
  }

  RenderLock renderLock;

  // Passage: a window of the chapter text centred on the current page's
  // proportional position. The whole chapter string is short-lived.
  std::string passage;
  {
    const std::string chapterText = section->getTextFromSectionFile();
    if (chapterText.empty()) {
      LOG_ERR("ASKB", "Current page has no text");
      return false;
    }
  ...
  const int tocIdx = epub->getTocIndexForSpineIndex(currentSpineIndex);
  std::string chapterName = (tocIdx >= 0) ? epub->getTocItem(tocIdx).title : "";
  std::string title = epub->getTitle();
  std::string author = epub->getAuthor();
  const int percent = bookPercentFor(chapterPosition());

  if (!saveProgress(currentSpineIndex, currentPage, totalPages)) {
    LOG_ERR("ASKB", "Aborting: current progress could not be saved");
    return false;
  }

  // Same release sequence as launchKOReaderSync(): TLS needs the heap the EPUB holds.
  LOG_DBG("ASKB", "Releasing epub (heap before: %u)", (unsigned)ESP.getFreeHeap());
  nextPageNumber = section->currentPage;
  discardOverlayPage();
  ImageBlock::releaseRenderCache();
  ImageBlock::setExtractor(nullptr, nullptr);
...
void EpubReaderActivity::renderStatusBar() const {
  ...
  } else if (sb.titleMode == InkAgentSettings::STATUS_BAR_TITLE::CHAPTER_TITLE) {
  ...
  } else if (sb.titleMode == InkAgentSettings::STATUS_BAR_TITLE::BOOK_TITLE) {
...
static_assert(std::size(kSpacingIds) == InkAgentSettings::LINE_COMPRESSION_COUNT, "line spacing labels");
static_assert(std::size(kAlignIds) == InkAgentSettings::PARAGRAPH_ALIGNMENT_COUNT, "alignment labels");
}  // namespace
...
bool EpubReaderActivity::usesToolbarMenu() const {
  ...
  return mappedInput.hasTouch() && SETTINGS.readerMenuStyle == InkAgentSettings::READER_MENU_TOOLBAR;
}
...
std::string EpubReaderActivity::textRowValue(int row) const {
  ...
  switch (row) {
    case 0:
      ...
      return I18N.get(kFamily[SETTINGS.fontFamily % InkAgentSettings::FONT_FAMILY_COUNT]);
    case 1:
      return std::to_string(SETTINGS.fontPointSize) + " pt";
    case 2:
      return I18N.get(kSpacingIds[SETTINGS.lineSpacing % InkAgentSettings::LINE_COMPRESSION_COUNT]);
    case 3:
      return I18N.get(kAlignIds[SETTINGS.paragraphAlignment % InkAgentSettings::PARAGRAPH_ALIGNMENT_COUNT]);
    case 4:
  ...
  switch (row) {
    ...
    case 2:
      overlayPopup.show(StrId::STR_LINE_SPACING, kSpacingIds, static_cast<int>(std::size(kSpacingIds)),
                        SETTINGS.lineSpacing % InkAgentSettings::LINE_COMPRESSION_COUNT, [this](int idx) {
    ...
    case 3:
      overlayPopup.show(StrId::STR_PARA_ALIGNMENT, kAlignIds, static_cast<int>(std::size(kAlignIds)),
                        SETTINGS.paragraphAlignment % InkAgentSettings::PARAGRAPH_ALIGNMENT_COUNT, [this](int idx) {
...
std::string EpubReaderActivity::moreRowValue(int row) const {
  ...
  static_assert(std::size(kOrient) == InkAgentSettings::ORIENTATION_COUNT, "orientation labels");
  if (row < 0 || row >= static_cast<int>(moreItems.size())) return "";
  switch (moreItems[row].action) {
    case MA::ROTATE_SCREEN:
      return I18N.get(kOrient[SETTINGS.orientation % InkAgentSettings::ORIENTATION_COUNT]);
...
void EpubReaderActivity::activateMoreRow(int row) {
  ...
  switch (action) {
    case MA::ROTATE_SCREEN: {
      ...
      static_assert(std::size(kOrientIds) == InkAgentSettings::ORIENTATION_COUNT, "orientation options");
      overlayPopup.show(StrId::STR_ORIENTATION, kOrientIds, static_cast<int>(std::size(kOrientIds)),
                        SETTINGS.orientation % InkAgentSettings::ORIENTATION_COUNT, [this](int idx) {
...
InkAgentPosition EpubReaderActivity::getCurrentPosition() const {
  const int currentPage = section ? section->currentPage : nextPageNumber;
  const int totalPages = section ? section->estimatedTotalPages() : cachedChapterTotalPageCount;
  ...
  InkAgentPosition localPos = {currentSpineIndex, currentPage, totalPages};
