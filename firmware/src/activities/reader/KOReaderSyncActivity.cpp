...
KOReaderSyncActivity::KOReaderSyncActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
                                           const std::string& epubPath, InkAgentPosition localPosition,
...
void KOReaderSyncActivity::ensureEpubLoaded() {
  if (!epub) {
    ...
    epub = std::make_shared<Epub>(epubPath, "/.inkagent");
    epub->setupCacheDir();
...
void KOReaderSyncActivity::onWifiSelectionComplete(const bool success) {
  ...
  // KOSync requests from InkAgent do not include a client timestamp.
  performSync();
}
...
void KOReaderSyncActivity::performSync() {
...

  {
    ...
    const auto mapRemoteProgress = [&](const KOReaderProgress& progress) {
      ...
      // The InkAgent server's existing rich page hints remain a legacy fallback.
      const SavedProgressPosition koPos = {progress.progress, progress.percentage};
      InkAgentPosition mapped =
          ProgressMapper::toInkAgent(epub, koPos, renderer, localPosition.spineIndex, localPosition.totalPages);
      if (!mapped.hasVisibleTextOffset && progress.position.has_value()) {
        // toInkAgent above already tried koPos.xpath; if the rich position carries the same XPath,
    ...
    if (hasAlternateProgress) {
      const InkAgentPosition alternatePosition = mapRemoteProgress(alternateProgress);
...
void KOReaderSyncActivity::performUpload() {
  ...
  // Rich InkAgent position for the default InkAgent sync server (lossless
  // InkAgent<->InkAgent sync). The HTTP client also enforces this boundary
  // before serializing the extension.
  if (KOREADER_STORE.usesInkAgentSyncServer()) {
