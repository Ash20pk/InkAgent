...
InkAgentWebServer* wsInstance = nullptr;
...
InkAgentWebServer::InkAgentWebServer() {}

InkAgentWebServer::~InkAgentWebServer() { stop(); }

void InkAgentWebServer::begin() {
  if (running) {
  ...
  wsInstance = const_cast<InkAgentWebServer*>(this);
  wsServer->begin();
...
void InkAgentWebServer::abortWsUpload(const char* tag) {
...
void InkAgentWebServer::handleClient() {
    ...
    if (packetSize > 0) {
      ...
      if (len > 0) {
        ...
        if (strcmp(buffer, "hello") == 0) {
          ...
          String message = "inkagent (on " + hostname + ");" + String(wsPort);
...
InkAgentWebServer::WsUploadStatus InkAgentWebServer::getWsUploadStatus() const {
...
void InkAgentWebServer::handleStatus() const {
  // Get correct IP based on AP vs STA mode
  const String ipAddr = apMode ? WiFi.softAPIP().toString() : WiFi.localIP().toString();

  JsonDocument doc;
  doc["version"] = INKAGENT_VERSION;
  doc["ip"] = ipAddr;
...
void InkAgentWebServer::scanFiles(const char* path, const std::function<void(FileInfo)>& callback) const {
...
bool InkAgentWebServer::isEpubFile(const String& filename) const { return FsHelpers::hasEpubExtension(filename); }

void InkAgentWebServer::handleFileList() const {
  sendStaticContent(server.get(), FilesPageHtml, sizeof(FilesPageHtml), FilesPageHtmlETag, "text/html");
}

void InkAgentWebServer::handleFileListData() const {
...
void InkAgentWebServer::handleDownload() const {
  if (!server->hasArg("path")) {
...
static bool flushUploadBuffer(InkAgentWebServer::UploadState& state) {
...
void InkAgentWebServer::handleUpload(UploadState& state) const {
...
void InkAgentWebServer::handleUploadPost(UploadState& state) const {
  if (state.success) {
...
void InkAgentWebServer::handleCreateFolder() const {
...
void InkAgentWebServer::handleRename() const {
...
void InkAgentWebServer::handleMove() const {
...
void InkAgentWebServer::handleDelete() const {
...
void InkAgentWebServer::handleGetSettings() const {
...
void InkAgentWebServer::handlePostSettings() {
...
void InkAgentWebServer::handleGetOpdsServers() const {
...
void InkAgentWebServer::handlePostOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void InkAgentWebServer::handleDeleteOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void InkAgentWebServer::handleGetWifiNetworks() const {
...
void InkAgentWebServer::handlePostWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
// Uses POST (not HTTP DELETE) because ESP32 WebServer doesn't support DELETE with body.
void InkAgentWebServer::handleDeleteWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void InkAgentWebServer::wsEventCallback(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  if (wsInstance) {
...
void InkAgentWebServer::onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
...
void InkAgentWebServer::handleFontUploadData() {
  ...
  switch (upload.status) {
    case UPLOAD_FILE_START: {
      ...
      // this an attacker could supply "../../.inkagent/settings.json" as
...
void InkAgentWebServer::handleFontUpload() {
  if (fontUpload.valid) {
...
void InkAgentWebServer::handleFontDelete() {
