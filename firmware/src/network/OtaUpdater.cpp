...
namespace {
constexpr char latestReleaseUrl[] = "https://api.github.com/repos/ash20pk/inkagent-firmware/releases/latest";
}  // namespace

OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  LOG_DBG("OTA", "Checking for update (current: %s)", INKAGENT_VERSION);
  ...
  // Each board updates from inkagent-<version>-<device>.bin. The combined
  ...
  const bool ok = HttpDownloader::fetchUrl(latestReleaseUrl, [&](const uint8_t* data, size_t len) {
    ...
    while (!assetNameSet && offset < len) {
      ...
      if (releaseParser.foundTag()) {
        snprintf(assetName, sizeof(assetName), "inkagent-%s%s.bin", releaseParser.getTagName(), assetSuffix);
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == INKAGENT_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = INKAGENT_VERSION;
