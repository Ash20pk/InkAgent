...
constexpr char SLEEP_FRAME_FILE[] = "/.inkagent/sleep_frame.bin";
...
void setup() {
...
#ifdef INKAGENT_WAIT_FOR_USB_SERIAL
  ...
  if (gpio.hasTouch()) {
    SETTINGS.readerMenuStyle = InkAgentSettings::READER_MENU_TOOLBAR;
  }
  ...
  switch (wakeupReason) {
    case HalGPIO::WakeupReason::PowerButton:
      ...
      if (!wakeHoldVerified && SETTINGS.shortPwrBtn != InkAgentSettings::SHORT_PWRBTN::SLEEP) {
...
void loop() {
  ...
  gpio.setSharedConfirmPowerShortPressEmitsPower(SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::SLEEP);
  ...
  if (SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::PWR_CONFIRM && BoardConfig::isX4Pro() &&
  ...
  if ((SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::SLEEP ||
       SETTINGS.shortPwrBtn == InkAgentSettings::SHORT_PWRBTN::IGNORE) &&
