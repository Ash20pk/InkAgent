...
This roadmap describes how InkAgent is moving through the tighter scope defined in [SCOPE.md](SCOPE.md). It is
...
InkAgent runs cleanly on ESP32-based e-reader hardware beyond Xteink (X3 / X4), including ESP32-S3 class devices.
...
* ~~Pluggable per-device SDK layers (display, input, storage, battery) and per-device build configuration without
  forking the reader core.~~ **Done.** InkAgent now builds for and runs on multiple device targets beyond the
...
The following are explicitly *not* on the roadmap. They may live in other InkAgent forks; they will not be picked
up here:
