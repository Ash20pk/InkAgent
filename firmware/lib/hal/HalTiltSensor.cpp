...
void HalTiltSensor::update(const uint8_t mode, const uint8_t orientation, const bool inReader) {
  ...
  if ((mode != InkAgentTiltPageTurn::TILT_OFF) && !_isAwake) {
    _isAwake = wake();
    return;
  } else if ((mode == InkAgentTiltPageTurn::TILT_OFF) && _isAwake) {
    _isAwake = !deepSleep();
    return;
  }

  // If disabled, skip the rest of the polling logic and avoid unnecessary I2C traffic in non-reader activities
  if ((mode == InkAgentTiltPageTurn::TILT_OFF) || !inReader) {
    return;
  }
  ...
  float tiltAxis;
  switch (orientation) {
    case InkAgentOrientation::PORTRAIT:
      tiltAxis = mode == InkAgentTiltPageTurn::TILT_INVERTED ? -gx : gx;
      break;
    case InkAgentOrientation::INVERTED:
      tiltAxis = mode == InkAgentTiltPageTurn::TILT_INVERTED ? gx : -gx;
      break;
    case InkAgentOrientation::LANDSCAPE_CW:
      tiltAxis = mode == InkAgentTiltPageTurn::TILT_INVERTED ? gy : -gy;
      break;
    case InkAgentOrientation::LANDSCAPE_CCW:
      tiltAxis = mode == InkAgentTiltPageTurn::TILT_INVERTED ? -gy : gy;
      break;
    default:
      tiltAxis = gx;
