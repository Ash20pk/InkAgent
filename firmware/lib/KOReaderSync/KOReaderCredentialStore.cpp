...
namespace {
// Default sync server URL. inkagent-sync speaks the full KOSync protocol, so
// pointing at any other kosync server (e.g. https://sync.koreader.rocks:443)
// still works via the custom server URL setting.
constexpr char DEFAULT_SERVER_URL[] = "https://sync.inkagentreader.com";
...
bool KOReaderCredentialStore::fromJson(JsonVariantConst doc) {
  ...
  // The default server changed in config v2 (sync.koreader.rocks -> inkagent-sync).
...
bool KOReaderCredentialStore::usesInkAgentSyncServer() const { return getBaseUrl() == DEFAULT_SERVER_URL; }
