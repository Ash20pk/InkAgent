...
/**
 * Rich InkAgent position sent alongside progress uploads. Maps 1:1 onto the
 * inkagent-sync extended `position` object (see inkagent-sync docs/API.md).
 * It is only transmitted to sync.inkagentreader.com. These fields remain
...
struct KOReaderProgress {
  ...
  std::optional<KOReaderRichPosition> position;  // Optional rich position (inkagent-sync servers only)
};
