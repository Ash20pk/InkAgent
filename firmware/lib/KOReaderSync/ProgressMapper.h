...
/**
 * Maps between InkAgent and SavedProgress position formats, such as those used by KOReader.
 *
 * InkAgent tracks position as (spineIndex, visibleTextOffset). Page number is
...
class ProgressMapper {
 public:
  /**
   * Convert InkAgent position to SavedProgress format.
   *
   * @param epub The EPUB book
   * @param pos InkAgent position
   * @return SavedProgress position
   */
  static SavedProgressPosition toSavedProgress(const std::shared_ptr<Epub>& epub, const InkAgentPosition& pos);

  /**
   * Convert SavedProgress position to InkAgent format.
   *
   ...
   * server) directly to a InkAgent position. Its standard KOReader XPath is
   ...
   *        toInkAgent(), so retrying it here would decompress the chapter twice for nothing.
