...
SavedProgressPosition ProgressMapper::toSavedProgress(const std::shared_ptr<Epub>& epub, const InkAgentPosition& pos) {
...
std::optional<InkAgentPosition> ProgressMapper::fromRichPosition(const std::shared_ptr<Epub>& epub,
  ...
  if (!xpathAlreadyTried && !rich.xpath.empty()) {
    ...
    auto contentMapped = toInkAgent(epub, saved, renderer);
...
InkAgentPosition ProgressMapper::toInkAgent(const std::shared_ptr<Epub>& epub, const SavedProgressPosition& koPos,
